...
def render_markdown(work_date: str, summary: dict | None,
    ...
    lines = [
        ...
        f"- **Claude:** {s.get('claude_messages', 0)} сообщ., "
        f"{s.get('claude_tokens', 0)} токенов · **коммитов:** {s.get('commits', 0)}",
        f"- **Гигиена процесса:** {s.get('hygiene_score', 0)} (доля коммитов с PLANE-ID)",
        "",
        "## Время по задачам",
    ...
    for t in tasks:
        ...
        lines.append(
            ...
            f"{t.get('claude_messages', 0)} | {t.get('claude_tokens', 0)} | {t.get('commits', 0)} |"
        )
    if not tasks:
        lines.append("| — | — | — | — | — | — |")
    if idles:
...
def render_csv(work_date: str, tasks: list[dict]) -> str:
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(["work_date", "task", "active_minutes", "est_h",
                "claude_messages", "claude_tokens", "commits"])
    for t in tasks:
        est = t.get("est_h")
        w.writerow([work_date, t.get("plane_identifier") or "", t.get("active_minutes") or 0,
                    ...
                    t.get("claude_tokens") or 0, t.get("commits") or 0])
    return buf.getvalue()
