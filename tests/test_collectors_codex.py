...
def _rollout_lines(sid="cx-1", cwd="C:\\dev\\proj", model="gpt-5.5", with_usage=True,
                   day="2026-06-09"):
    lines = [
        {"timestamp": f"{day}T08:00:00.100Z", "type": "session_meta",
         "payload": {"id": sid, "timestamp": f"{day}T08:00:00.000Z", "cwd": cwd,
                     "base_instructions": {"text": _BODY}}},
        {"timestamp": f"{day}T08:00:01Z", "type": "turn_context",
         "payload": {"turn_id": "t1", "cwd": cwd, "model": model}},
        {"timestamp": f"{day}T08:00:05Z", "type": "response_item",
...
def test_iter_sessions_since_filter(tmp_path):
    _write(tmp_path / "2026" / "05" / "20" / "rollout-old.jsonl",
           _rollout_lines(sid="old", day="2026-05-20"))
    _write(tmp_path / "2026" / "06" / "09" / "rollout-new.jsonl",
           _rollout_lines(sid="new"))
    got = iter_sessions(tmp_path, since="2026-06-01")
    assert [s.session_uid for s in got] == ["new"]  # май отфильтрован (путь + таймстемпы)


def test_iter_sessions_boundary_fractional_seconds(tmp_path):
    """started_at с долями секунд РОВНО на границе since не должен теряться.

    Лексикографически '08:00:00.100Z' < '08:00:00Z' (точка < Z) — ловушка строкового
    сравнения, найденная codex-ревью; сравниваем распарсенные таймстемпы.
    """
    _write(tmp_path / "2026" / "06" / "09" / "rollout-b.jsonl", _rollout_lines(sid="b"))
    got = iter_sessions(tmp_path, since="2026-06-09T08:00:00Z")  # старт 08:00:00.000Z
    assert [s.session_uid for s in got] == ["b"]


def test_iter_sessions_maturing_session_included(tmp_path):
    """Сессия, начатая ДО окна, но закончившаяся В окне, дозревает (включается)."""
    lines = _rollout_lines(sid="long", day="2026-06-07")
    lines.append({"timestamp": "2026-06-09T09:00:00Z", "type": "event_msg",
                  "payload": {"type": "token_count", "info": {
                      "total_token_usage": {"input_tokens": 9000, "cached_input_tokens": 0,
                                            "output_tokens": 900, "reasoning_output_tokens": 0,
                                            "total_tokens": 9900}}}})
    _write(tmp_path / "2026" / "06" / "07" / "rollout-long.jsonl", lines)
    got = iter_sessions(tmp_path, since="2026-06-09T00:00:00Z")
    assert [s.session_uid for s in got] == ["long"]  # ended_at в окне → дозревание
    assert got[0].input_tokens == 9000
...
def test_collector_codex_since_floor(tmp_path):
    sessions = tmp_path / "sessions"
    _write(sessions / "2026" / "05" / "20" / "rollout-may.jsonl",
           _rollout_lines(sid="may", day="2026-05-20"))
    repo = SqliteRepository.open(tmp_path / "db.sqlite")
    emp = repo.upsert_employee("Oleg")
...
def test_collector_recollect_updates_grown_session(tmp_path):
    """Повторный collect по выросшему rollout обновляет meta события и сессию."""
    sessions = tmp_path / "sessions"
    f = sessions / "2026" / "06" / "09" / "rollout-g.jsonl"
    lines = _rollout_lines(sid="grow")
    _write(f, lines)
    repo = SqliteRepository.open(tmp_path / "db.sqlite")
    emp = repo.upsert_employee("Oleg")
    c = CodexCollector(repo, sessions)
    ...
    lines.append({"timestamp": "2026-06-09T09:00:00Z", "type": "event_msg",
                  "payload": {"type": "token_count", "info": {
                      "total_token_usage": {"input_tokens": 7000, "cached_input_tokens": 2500,
                                            "output_tokens": 900, "reasoning_output_tokens": 120,
                                            "total_tokens": 7900}}}})
    _write(f, lines)
    c.collect(emp)
    evs = repo.events_between(emp, "2026-06-09T00:00:00Z", "2026-06-09T23:59:59Z")
    assert len(evs) == 1  # то же событие (идемпотентность), но с новым итогом
    meta = json.loads(evs[0]["meta_json"])
    assert meta["input"] == 7000 and meta["turns"] == 3
    sess = repo._fetchone("SELECT * FROM agent_session WHERE session_uid='grow'")
    assert sess["tokens_in"] == 7000 and sess["ended_at"] == "2026-06-09T09:00:00Z"
    repo.close()
