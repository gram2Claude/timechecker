...
- **Собственный реестр задач** (`timechecker task import/add/start/done/move/list`): задачи и переходы
  статусов пишутся напрямую в БД; переходы дают «окна в работе» для атрибуции времени.
- **«Прочие работы»** (схема v5): задача вне канона плана (`task add`) автоматически относится
  к «Прочим работам» соответствующего спринта (резолв по дате, справочник спринтов из `task import`;
  коррекция — `task move`). Инвариант: задача либо в плане, либо в «прочих» — третьего нет.
...
| `timechecker task import/add/start/done/list` | собственный реестр задач (канон → БД + справочник спринтов, переходы статусов) |
| `timechecker task add --slug … --title … [--sprint sX.Y]` | внеплановая задача → «Прочие работы» спринта (резолв по дате) |
| `timechecker task move ID --sprint sX.Y` | перепривязать внеплановую задачу к другому спринту |
| `timechecker task backfill-sprints [--slug]` | разовый бэкфилл привязки старых внеплановых задач |
