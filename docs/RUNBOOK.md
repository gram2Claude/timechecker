...
второго источника истины. Набор авторитетный (REVOKE ALL → ровно `SELECT` на 4 таблицы +
`INSERT/UPDATE` на `tg_chat_bindings`; на `public`/sequence — ничего).

### Нюансы взаимодействия с ботом (фиксировано ревью E11)
- **Unbind из кабинета — односторонняя защёлка.** Кабинет пишет `bound_via='cabinet'`, а бот
  апдейтит строку только `WHERE bound_via='bot'`. После кабинетного unbind/привязки бот **не
  перезапишет** эту строку (в т.ч. `chat_title`) — чат управляется только из кабинета (это и есть
  контрактное «кабинет — источник истины», B1). Переназначить чат боту обратно нельзя без ручного
  сброса `bound_via`.
- **`active` заполняет бот.** Колонка `active` (скрытие чата) в боевой схеме пока всегда `true`:
  кабинет её читает и показывает бейдж «скрыт», но **бот ещё не пишет** `active` в общую таблицу
  (трекает локально). До доработки бот-стороны бейдж «скрыт» не появляется — это не дефект кабинета.

### Лимиты длины контента (миграция v7, TIME-80, DoS-харднинг)
CHECK-констрейнты на схеме `tg_assistant` ограничивают длину каждой строки контента (защита от
раздувания → тяжёлый рендер раздела «Чаты»): `content_md` (digests/topics) ≤ 65536, `tg_journal.text`
и `norm_text` ≤ 8192, `tg_topics.name` ≤ 200, `chat_title` ≤ 256. **Контракт боту:** запись сверх
лимита отклоняется Postgres-ошибкой `23514` — бот должен **усекать** контент до лимита перед
отправкой (иначе строка зависнет в outbox с повторами). Кабинет дополнительно ограничивает выборку
(`getDigests`/`getTopics`/`getJournal` — `LIMIT`) и время запроса (`statement_timeout=15s` на роли
`nexus_admin_app`). Лимиты щедрые (реальный максимум контента ~1.6K) — легитимный контент не задет.
