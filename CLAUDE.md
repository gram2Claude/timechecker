...
**Метаданные-only:** тела сообщений Claude НЕ читаются и не хранятся.
...
- CLI: `uv run timechecker <cmd>` — `initdb · collect · metrics · report · daily · health · prune ·
  deploy · sync · migrate-db · register-project · projects · schedule · hook`.

## Архитектура
`collectors/` (Claude·hooks·git·Plane) → `storage/` (repository DAO) → `metrics/` → `reporting/` → `ops/`.
...
- **Секреты** (Plane/GitHub/Supabase DSN) — только в `~/.wgp/secrets.json`, НЕ в репозитории.
...
→ merge → push → Plane Done). Планирование/задачи — Plane (`workflow_global_plan`, проект `TIME`).
